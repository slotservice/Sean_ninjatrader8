...
| Risk Management *(added 2026-04-22 evening, project wrap-up)* | All four params default 0 (disabled). **Stop Loss (ticks)** — auto-attached hard stop, NT8 `SetStopLoss(CalculationMode.Ticks)`. For MNQ at Value=24 chart: 1 brick = 24 ticks = $12. **Profit Target (ticks)** — same mechanism. **Max Trades Per Day** — counter resets at session start. **Daily Loss Limit ($)** — force-flat + block new entries when today's session PnL ≤ -$X. Tracked via `SystemPerformance` cumulative profit minus session-start baseline + open-position unrealized PnL. Stop/Target apply via NT8's `State.Configure` — strategy must be disabled + re-enabled to apply param changes. |
